-------------lab7------------------
----Câu1------------- 
CREATE PROCEDURE Cau1 (@mahangsx INT, @tenhang VARCHAR(50), @diachi VARCHAR(50), @sodt VARCHAR(15), @email VARCHAR(50))
AS
BEGIN
...
END
EXEC Cau1 @mahangsx = 123, @tenhang = N'Samsung1', @diachi = N'Hàn Quốc', @sodt = N'0123456789', @email = N'contact@samsung.com'


-- cau 2
go
CREATE PROCEDURE ThemSuaSP
    @masp NVARCHAR(10),
...
go
go
EXEC Xoa_HangSX @tenhang = 'SamSung1'
go
...
go
go
EXEC ThemSua_Nhap 'HD001', 'SP001', 'NV001', '2023-04-07', 10, 50000.00
go

-- cau 7
CREATE PROCEDURE XoaNVien
    @manv INT
AS
BEGIN
    SET NOCOUNT ON;
...
-- cau 8
go
CREATE PROCEDURE Xoasp(@masp int)
AS
BEGIN
